...
main() {
  ...
  echo "  Fill..."
  fio --direct=1 --ioengine=libaio --iodepth=64 --filename=$DEV --name=test --rw=write --bs=1M --fill_device=1 >/dev/null

  echo "  PLM start...   $(date +"%Y-%m-%d %H:%M:%S.%N")"
  nvme admin-passthru /dev/nvme1 -n 1 -o 0x09 -w --cdw10=0x13 --cdw11=$NAMESPACE --cdw12=0x01
  echo "  DTWIN start...   $(date +"%Y-%m-%d %H:%M:%S.%N")"
  nvme admin-passthru /dev/nvme1 -n 0x1 -o 0x09 -w --cdw10=0x14 --cdw11=$NAMESPACE --cdw12=0x01
  ...
  --log_avg_msec=1 --write_lat_log=${BLKTRACE_RESULT_PATH}/lat.log --write_bw_log=${BLKTRACE_RESULT_PATH}/bw.log --output=${ROOT_PATH}/fio.log >/dev/null &

  # window check
  while [ 1 -eq 1 ]; do
    window=$(nvme get-feature $DEV -n 1 -f 20 -c 4 | tail -c 2)

    if [ $window == "2" ]; then
        nvme admin-passthru $DEV -n 0x1 -o 0x09 -w --cdw10=0x14 --cdw11=0x04 --cdw12=0x01
